...
export class StatsComponent {
  ...
  avgDistance = computed(() =>
    this.filteredRuns().length
      ? this.totalDistance() / this.filteredRuns().length
      : 0,
  );

  longestRun = computed(() =>
    this.filteredRuns().length
      ? Math.max(...this.filteredRuns().map((r) => Number(r.distance)))
      : 0,
  );

  fastestPace = computed(() =>
    this.filteredRuns().length
      ? Math.min(...this.filteredRuns().map((r) => Number(r.pace)))
      : 0,
  );
  ...
  maxMonthlyDistance = computed(() =>
    this.monthlyData().length
      ? Math.max(...this.monthlyData().map((d) => d.distance))
      : 1,
  );
  ...
  weeklyData = computed(() => {
    ...
    const runs =
      this.filterMode() === 'all' ? this.svc.runs() : this.filteredRuns();
    for (const run of runs) {
      const parts = (run.date as string).split('-');
      const d = new Date(+parts[0], +parts[1] - 1, +parts[2]);
      const startOfWeek = new Date(d);
      startOfWeek.setDate(d.getDate() - d.getDay());
      const key = startOfWeek.toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
      });
  ...
  maxWeeklyDistance = computed(() =>
    this.weeklyData().length
      ? Math.max(...this.weeklyData().map((d) => d.distance))
      : 1,
  );
  ...
  paceData = computed(() =>
    [...this.filteredRuns()]
      .sort((a, b) => {
        ...
        return (
          new Date(+pa[0], +pa[1] - 1, +pa[2]).getTime() -
  ...
  maxShoeMiles = computed(() =>
    this.shoeStats().length
      ? Math.max(...this.shoeStats().map((s) => s.miles))
      : 1,
  );
  ...
  scatterPoints = computed(() => {
    ...
    const paces = runs.map((r) => Number(r.pace));
    ...
    return runs.map((r) => ({
      x:
        maxDist === minDist
          ? (xStart + xEnd) / 2
          : xStart +
            ((Number(r.distance) - minDist) / (maxDist - minDist)) *
              (xEnd - xStart),
      y:
        yEnd -
        ((Number(r.pace) - minPace) / (maxPace - minPace)) * (yEnd - yStart),
      distance: Number(r.distance),
      pace: Number(r.pace),
      label: `${Number(r.distance).toFixed(1)}mi @ ${this.formatPace(Number(r.pace))}`,
    }));
  });

  scatterXLabels = computed(() => {
    const runs = this.filteredRuns();
    if (!runs.length) return [];
    const distances = runs.map((r) => Number(r.distance));
    const min = Math.min(...distances);
    const max = Math.max(...distances);
    return [0, 1, 2, 3, 4].map((i) => (min + (i / 4) * (max - min)).toFixed(1));
  });

  scatterYLabels = computed(() => {
    const runs = this.filteredRuns();
    if (!runs.length) return [];
    const paces = runs.map((r) => Number(r.pace));
    const minP = Math.min(...paces) - 0.5;
  ...
  // Correlation — positive means longer = slower, negative means longer = faster
  paceDistanceCorrelation = computed(() => {
    const runs = this.filteredRuns();
    ...
    const num = distances.reduce(
      (s, d, i) => s + (d - avgDist) * (paces[i] - avgPace),
      0,
    );
    const den = Math.sqrt(
      distances.reduce((s, d) => s + (d - avgDist) ** 2, 0) *
        paces.reduce((s, p) => s + (p - avgPace) ** 2, 0),
    );
